<?php
	require "db.php";
	if (isset($_POST['send'])){
		if(trim($_POST[name])=="" || trim($_POST[subject])=="" || trim($_POST[message])==""){
			$err = 'Заполните все поля!';
		}
	else{
		$komments = R::dispense('komment');
		$komments->name = $_POST['name'];
		$komments->subject = $_POST['subject'];
		$komments->message = $_POST['message'];
		$komments->date = date("d.m.Y");
	
		R::store($komments);
		header('location: komments.php');
		}
	}
?>
...
<html lang="en">
	<head>
		...
		<link rel="stylesheet" href="css\kom.css" type="text/css">
		<style type="text/css">
...
<!---------------Header---------------->
					...
					<div class="container">
						...
						<div class="navbar-collapse collapse">
							<ul class="nav navbar-nav">
								...
								<li class="active"> <a href="komments.php" class="smoothScroll"> Оставить отзыв</a></li>
							</ul>
		...
		<div class=".form">
			<form action="" method="post">	
				<label><b>Оставить отзыв:</b></label><br/><br/>
				<input class="inp" type="text" name="name" placeholder="Имя"><br/><br/>
				<input class="inp"type="text" name="subject" placeholder="Тема"><br/><br/>
				<textarea type="text" name="message" placeholder="Комментарий"></textarea><br/><br/>		
				<div class="g-recaptcha" data-sitekey="6Ld2MVoUAAAAADd1nXWCrPF6ISXEtVxWOr3nU_Ew"></div><br/>
				<?= '<div class="err" style="color: red">'.$err.'</div>' ?>
				<button type="submit" name="send" class="submit btn">Отправить</button>		
			</form>	
		</div>

<!------------Вывод коммкнтариев--------------->		
			<?php $result = mysql_query('SELECT * FROM `komment`', $conn); // запрос на выборку
			if(!$result){
			...
			while($row = mysql_fetch_array($result)){
				echo
					'<div class="komment">
						<div class="i">'.$row['date'].'</div>			
						<div class="name">'.$row['name'].'</div><hr/>			
						<div class="subject">'.$row['subject'].'</div><br/>			
